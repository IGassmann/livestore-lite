...
LiveStore examples ship via Cloudflare Workers using the `pnpm run examples:deploy` workflow. The script builds each example and deploys the corresponding Worker. Each deployment is reachable via its `*.livestore.workers.dev` hostname—no additional DNS step is required.
...
- Verify that the target Worker does not already exist or can be replaced. `pnpm run examples:deploy` emits the Workers.dev URL so you can sanity-check the new build.
