...
```bash
CI=1 vp run -w release:snapshot -- --dry-run --yes
```

To dry-run a snapshot for a specific commit:

```bash
CI=1 vp run -w release:snapshot -- --git-sha=<git-sha> --dry-run --yes
```
...
After the release task writes the fixed public package versions, the release PR
generator also syncs standalone examples and other non-workspace consumers to
the exact release version. This keeps `vp install --lockfile-only` validating
...
```bash
vp run -w release:notes:extract
```
...
The prod docs deploy was previously a single `vp run -w docs:deploy:prod`
...
under the shared `OTEL_EXPORTER_OTLP_ENDPOINT` wired by the release workflow setup.
...
```bash
LIVESTORE_NPM_TAG=latest vp run -w release:changeset:version
```
...
```bash
CI=1 vp run -w release:stable:dryrun
```

Dry-run the DevTools artifact repack for the planned version:

```bash
CI=1 LIVESTORE_RELEASE_VERSION=0.4.0 vp run -w release:devtools-artifact:repack-dryrun
```
...
3. Run `CI=1 vp run -w release:devtools-artifact:verify`.
...
- Treat `release/release-plan.json` as reviewable release intent, not a scratch
  file.
- Treat `release/version.json` as the source of truth for the checked-in
...
- Keep checked-in release-plan PRs installable before publish. The stable
  ...
  validated.
- Regenerate generated workflow files through the former generator; do not edit generated
