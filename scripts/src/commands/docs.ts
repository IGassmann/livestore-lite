 ...
 * Chromium helpers alive after work completes, keeping `vp run -w docs:build:phase:astro`
    ...
    Cli.Command.make(
      ...
      Effect.fn('docs.preview')(function* ({ port: portOption, build }) {
        ...
        if (fs.existsSync(distPath) === false) {
          yield* Effect.logWarning(
            `Docs dist folder not found at ${distPath}. Run 'vp run -w docs:build:phase:astro' or pass '--build' to the preview command.`,
          )
        }
