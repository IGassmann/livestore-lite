...
- `vp run -w docs:dev` / `vp run -w docs:build:phase:astro` — common docs workflows
- `vp run --filter <example-package> test` — example validation
- `vp exec changeset status --since origin/main` — inspect release status
...
`vp config --hooks --no-agent`. This sets `core.hooksPath=.vite-hooks/_`;
...
- `vp run -w check:all` — ensure static checks pass
