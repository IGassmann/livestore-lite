...
`megarepo` CLI, OTEL dashboards, and
...
runs the project `setup` task (which installs pnpm deps and builds TypeScript).
...
- `devenv shell <cmd> [args...]` — run a single command inside the
  ...
  CI (`devenv shell pnpm typecheck`).
- `pnpm run` — list package scripts exposed by `package.json` (see
  [Common commands](#common-commands) below for the most-used ones).
- `pnpm run <task>` — run one of the repo task scripts directly. The project's
  pre-commit hook uses this (`pnpm run check:quick`).
...
- `pnpm typecheck` — build/check TypeScript across the monorepo
- `pnpm lint` / `pnpm lint:fix` — run or auto-fix lint + format checks
- `pnpm test:unit` / `pnpm test:integration` / `pnpm test:perf` — run test suites
- `pnpm docs:dev` / `pnpm docs:build` — common docs workflows
- `pnpm examples:test` — example validation
- `pnpm run release:changeset:status` — inspect release status
- `pnpm run task -- --help` — list compatibility task shims

Most package scripts delegate to the repo's `mono` command or to a small
compatibility shim in `scripts/src/task-shims.ts`. The old devenv task names are
available as package scripts, so use `pnpm run <task-name>` for task-style
workflows.
...
- `pnpm typecheck` — ensure TypeScript passes
- `pnpm test:unit` (and relevant integration suites) — run tests
