...
Use `pnpm exec vp run -w` to open the interactive task picker, and run repository
tasks with `pnpm exec vp run -w <task>`.
...
- `pnpm exec vp run -w ts:check` — build/check TypeScript across the monorepo
- `pnpm exec vp run -w lint:full` / `pnpm exec vp run -w lint:full:fix` — run or auto-fix lint + format checks
- `pnpm exec vp run -w test:unit` / `pnpm exec vp run -w test:integration` / `pnpm exec vp run -w test:perf` — run test suites
- `pnpm exec vp run -w docs:dev` / `pnpm exec vp run -w docs:build` — common docs workflows
- `pnpm exec vp run -w examples:test` — example validation
- `pnpm exec vp run -w release:changeset:status` — inspect release status
- `pnpm exec vp run -w` — open the interactive task picker

Root task definitions live in `vite.config.ts`. Use
`pnpm exec vp run -w <task-name>` for task-style workflows.
...
A `pre-commit` hook runs `pnpm exec vp run -w check:quick` automatically, but you should
also run the following manually for a tighter feedback loop:

- `pnpm exec vp run -w lint:full:fix` — auto-fix lint + format issues
- `pnpm exec vp run -w ts:check` — ensure TypeScript passes
- `pnpm exec vp run -w test:unit` (and relevant integration suites) — run tests
