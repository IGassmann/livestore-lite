 ...
 * This command runs before `pnpm exec vp run -w docs:build` to guarantee cached HTML is available during Astro builds.
 */
    ...
    const buildSnippet = (entry: TSnippetEntry) =>
      Effect.gen(function* () {
        ...
        for (const filename of bundle.fileOrder) {
          ...
          const rendered = yield* shouldRenderTwoslash === true
