...
- **A02 Shared tooling source:** `overengineeringstudio/effect-utils` supplies
  canonical devenv modules, CI workflow patterns, label/repo settings helpers,
...
- **A04 Single docs site:** `docs.livestore.dev` is the canonical
...
- **R16 Same devenv stack:** Contrib imports the same effect-utils devenv
  modules as core.
- **R17 Shared config patterns, local ownership manifests:** Contrib's
  workspace, package, TypeScript, lint/format, CI, labels, and repo settings
  files follow core/effect-utils patterns rather than handwritten copies. Final
